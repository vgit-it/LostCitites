...
3 digits, `100`–`999`. Generated by the tablet itself, on first load (`client/src/main.tsx`) — the wire views carry no code field, so a server-invented code could never reach the tablet to display it. Displayed large on the tablet. A refresh reuses the stored code rather than minting a new one.

Do not use `0` as a leading digit — avoids ambiguity when someone reads it aloud.

Also scanned. The tablet already knows the LAN address it was loaded from, so each open seat in the lobby carries a QR encoding `/play?code=…&seat=…` (`client/src/shared/invite.ts`, `client/src/table/qrCode.ts`) — scanning lands a phone on the join screen with the code and seat already filled in, name left to type. Typing the code by hand stays the fallback; no server change was needed to support it.
